...
public class V
{
    ...
    final String pRT, final int pNP, final String pExist )
    {
        ...
        String vMessage = ( vWanted ? "n'existe pas" : "ne devrait pas exister" );
        MethodsContent vLMet = new MethodsContent( pCla.className() );
        vrai( vLMet.hasMethodRTNP( pMethod, pRT, pNP ) == vWanted, "$a avec $n parametre(s) " + vMessage + " !" );
        failIfNot();
    } // verifMetRTNP(.....)
    ...
    public static void verifParamPNType( String pFil, String pMet, String pType, int pN ) // Neme parametre
    {
        ...
        String vP = "p";
        final String vParamA = "\\s*"+"final\\s+"+"\\w+"+"\\s+"+vP+"\\w+\\s*";
        vP = "";
        final String vParamS = "\\s*"+"final\\s+"+"\\w+"+"\\s+"+vP+"\\w+\\s*";

        String vAvec = vDebut;
        for ( int vI=1; vI<pN; vI++ )   vAvec += vParamS + "\\,";
        vAvec += vParamA + vFin;

        String vSans = vDebut;
        for ( int vI=1; vI<pN; vI++ )   vSans += vParamS + "\\,";
        vSans += vParamS + vFin;
